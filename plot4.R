# Read in data, rename it and subset parameters by date and column
power <- read.table("household_power_consumption.txt",skip=1,sep=";")
names(power) <- c("Date","Time","Global_active_power","Global_reactive_power","Voltage","Global_intensity","Sub_metering_1","Sub_metering_2","Sub_metering_3")
subpower <- subset(power,power$Date=="1/2/2007" | power$Date =="2/2/2007")

# Modify the date and time into Date and POSIXlt
...
# Set up composite plot with four individual graphs
par(mfrow=c(2,2))

# Calling plot function that calls different plot funcs building 4 plots that form the graph
with(subpower,{
